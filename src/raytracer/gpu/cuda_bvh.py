...
def _leaf_hit_t(
    ...
    spheres, planes, triangles, bvh_prims,
    t_min, t_max,
):
    ...
    return _triangle_hit_t(
        ox, oy, oz, dx, dy, dz,
        triangles[pindex, 0], triangles[pindex, 1], triangles[pindex, 2],
        triangles[pindex, 3], triangles[pindex, 4], triangles[pindex, 5],
        triangles[pindex, 6], triangles[pindex, 7], triangles[pindex, 8],
        t_min, t_max,
    )
...
def bvh_any_hit(
    ...
    while stack_ptr > 0:
        ...
        if left < _F0:
            ...
            t = _leaf_hit_t(
                prim_idx, ox, oy, oz, dx, dy, dz,
                spheres, planes, triangles, bvh_prims, t_min, t_max,
            )
            if t > _F0:
                return True
            continue
    ...
    while stack_ptr > 0:
        ...
        if left < _F0:
            ...
            t, px, py, pz, nx, ny, nz, ff, mat_idx = _leaf_closest_hit(
                prim_idx, ox, oy, oz, dx, dy, dz,
                spheres, planes, triangles, bvh_prims, t_min, closest_t,
            )
            if t > _F0:
                closest_t = t
