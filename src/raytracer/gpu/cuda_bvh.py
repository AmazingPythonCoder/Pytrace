...
def direct_find_closest_hit(
    ...
    for i in range(spheres.shape[0]):
        ...
        if t > _F0:
            ...
            best_py = oy + t * dy
            best_pz = oz + t * dz
            outward_nx = (best_px - spheres[i, 0]) / spheres[i, 3]
            outward_ny = (best_py - spheres[i, 1]) / spheres[i, 3]
            outward_nz = (best_pz - spheres[i, 2]) / spheres[i, 3]
            best_ff = 1 if (dx * outward_nx + dy * outward_ny + dz * outward_nz) < _F0 else 0
            best_nx = outward_nx if best_ff else -outward_nx
            best_ny = outward_ny if best_ff else -outward_ny
            best_nz = outward_nz if best_ff else -outward_nz
            best_mat = spheres[i, 4]
